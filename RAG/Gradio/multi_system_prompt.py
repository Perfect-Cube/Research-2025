...
from transformers import pipeline, AutoProcessor, AutoModelForVision2Seq, MarianMTModel, MarianTokenizer
...
# Load Embedding Model to obtain its dimension
...
root_directory = "C:/Users/alukkib/Documents/Aya Vision/Standards und Normen"  # Change this to your folder path
...
while True:
    ...
    # Determine query language for retrieval and system prompt
    try:
        query_lang = detect(query)
    except Exception:
        query_lang = "en"

    # Set system prompt based on query language
    if query_lang == "de":
        system_prompt_text = (
            "You are an AI agent of Volkswagen. Give detailed answers to the user's questions "
            "using the retrieved text and image only. Respond in German."
        )
    else:
        system_prompt_text = (
            "You are an AI agent of Volkswagen. Give detailed answers to the user's questions "
            "using the retrieved text and image only. Respond in English."
        )
    ...
    if show_image == "yes":
        ...
        print("Rendered images:", img_list)

    user_prompt = input("Enter additional prompt for text generation (or press Enter to skip): ").strip()
    if user_prompt:
        # If the additional prompt is in German, translate it to English before feeding to the generation model.
        try:
    ...
    # Combine user prompt with the retrieved texts for context
