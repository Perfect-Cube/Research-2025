...
def generate_text(prompt, search_results):
    ...
    combined_prompt = f"Please respond in English.\n{prompt}\n{combined_text}"
    conversation = [{
        "role": "user",
...
def generate_text_final(prompt, image_option, gallery, search_results):
    # If the user opted for image-based generation and a gallery image is selected:
    if image_option.lower() == "yes" and gallery:
        # 'gallery' returns a list of file paths (OS paths)
        ...
        if selected is not None:
            ...
            for i in selected:
                if i is not None:
                    img=i
                    break
            os_path = os.path.abspath(img).replace("\\","/")
            print("Selected OS image path:", os_path)
            # Open the image using the OS path
            selected_image = Image.open(os_path)
            # Wrap the image in a list to match the expected input format for the processor
            return generate_text_from_image(prompt, os_path)
    # Otherwise, fallback to text-based generation using the retrieved search results
...
def preview_images(search_results):
    ...
    for res in search_results:
        ...
        if pdf_path:
            # Convert 1-indexed page to 0-indexed for rendering.
...
with gr.Blocks(theme="default") as demo:
    ...
    with gr.Tabs():
        ...
        with gr.Tab("Search & Generate"):
            ...
            # Single generate text option that checks the image_option.
